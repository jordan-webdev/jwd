...
function acf_load_social_media_choices($field)
{
    ...
    if (have_rows('social_media_options', 'option')) {
        while (have_rows('social_media_options', 'option')) {
            the_row();

            $label = get_sub_field('social_media_options_name');
            $value = get_sub_field('social_media_options_link');
